...
for col in cols:
    orders_dataset_df[col] = orders_dataset_df[col].fillna(0)
orders_dataset_df.info()
...
# Mengaktifkan sidebar di Streamlit (semua kode di bawah with st.sidebar: akan muncul di sidebar).
# Menampilkan gambar/logo di sidebar (ambil dari link GitHub).
with st.sidebar:
